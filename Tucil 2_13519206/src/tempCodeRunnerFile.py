...
def bacaFile(namafile):
    ...
    try:
        ...
        f = open(directory, "r")
        lines = f.readlines()
        courses = []    # list yang menampung sub-array matkul
        for line in lines:
            course = [] # list yang akan ditampung (sub-array) pada list courses. berisi matkul dan prasyarat
            # Menghapus karakter yang tidak diperlukan/yang dapat menganggu program. seperti quote ('') dsb.
            x = re.sub(r'[^\w\s]', '', line)
            y = re.sub(r'\n', '', x)
            if y!="":
                y = y.split(' ')
...
def topSort(coursesGraph):
    ...
    while(len(coursesGraph)!=0):
        selectedCourse = [] # Menampung course (simpul) dengan busur masuk = 0
        for course, prereq in coursesGraph.items():
            if(len(prereq)==0):
